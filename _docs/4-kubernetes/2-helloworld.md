...
1. Bring up the application containers:
   ...
   ```bash
   $ kubectl create -f examples/k8s/k8s-helloworld.yaml
   ```
...
1. Lets send all traffic to the v1 version of helloworld:
   ...
   ```bash
   $ kubectl create -f examples/k8s/k8s-helloworld-default-v1.yaml
...
1. Next, we will split traffic between helloworld v2 (25%) and v1 (the remaining 75%)
   ...
   ```bash
   $ kubectl create -f examples/k8s/k8s-helloworld-25p-v2.yaml
   ```
...
1. Delete the routing rules
   ...
   ```bash
   $ kubectl delete -f examples/k8s/k8s-helloworld-default-v1.yaml
   routingrule "set-helloworld-v1-default" deleted
   $ kubectl delete -f examples/k8s/k8s-helloworld-25p-v2.yaml
   ```

1. Terminate the application pods

   ```bash
   $ kubectl delete -f examples/k8s/k8s-helloworld.yaml
   ```
